...
function ulTest(done) {
	...
	req = new Blob(req, { type: "text/plain" });
	...
	reqsmall.push(r);
	reqsmall = new Blob(reqsmall, { type: "text/plain" });
			...
			setTimeout(
				function() {
					...
					if (ie11workaround) {
						...
						xhr[i].setRequestHeader("Content-Type", "text/plain"); // Bypass CORS preflight for Cloudflare
						xhr[i].send(reqsmall);
					} else {
